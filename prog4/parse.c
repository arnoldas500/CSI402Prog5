...
int parse_ts_instruction(char *instruction) {
  ...
  if (opcode_val == -1) {
    add_error(lineNum, 'I', "Illegal opcode.");
  }
  ...
  switch (format_by_opcode((unsigned int) opcode_val)) {
    case 'R':
      rInstruct((unsigned int) opcode_val, args);
      break;

    case 'I':
      iInstruct((unsigned int) opcode_val, args, nargs);
      break;

    case 'J':
      jInstruct((unsigned int) opcode_val, args, nargs);
      break;
  }
